...
@testset "Test set for newton_ldlt_armijo" begin
	...
	sol = with_logger(NullLogger()) do 
	    newton_ldlt_armijo(himmelblau, himmelblau.meta.x0) 
	end
	...
	sol = with_logger(NullLogger()) do 
	    newton_ldlt_armijo(problem2, problem2.meta.x0) 
	end
	...
	sol = with_logger(NullLogger()) do 
	    newton_ldlt_armijo(rosenbrock, rosenbrock.meta.x0) 
	end
	...
	sol = with_logger(NullLogger()) do 
	    newton_ldlt_armijo(pb_du_cours, [-1.001, -1.001]) 
	end
	# @test stats.status == :unbounded
    @test obj(pb_du_cours, sol) ≤ -1e15
	@show sol

    ep4 = 1e-6 + norm(grad(pb_du_cours, [1.5, .5])) * 1e-6
	sol = with_logger(NullLogger()) do 
	    newton_ldlt_armijo(pb_du_cours, [1.5, .5]) 
	end
	...
	sol = with_logger(NullLogger()) do 
	    newton_ldlt_armijo(pb_du_cours, [.5, .5]) 
	end
	# @test stats.status == :unbounded
    @test obj(pb_du_cours, sol) ≤ -1e15
	@show sol
